...
import { getValue, getUnit, initUnitValue } from '../src/js/components/unitValue';
...
describe('Unit', () => {
  ...
  it('switches from px to % and value > 100, updates to 100', () => {
    const pxBtn = document.querySelector('[data-unit="px"]');
    pxBtn.click();
    ...
    expect(valueInput.value).toBe('100.0');
  });
});
...
describe('Value stepper - Input validation', () => {
  ...
  it('removes invalid characters: 123a -> 123', () => {
    ...
    expect(valueInput.value).toBe('123.0');
  });
  ...
  it('removes invalid characters: 12a3 -> 12', () => {
    ...
    expect(valueInput.value).toBe('12.0');
  });
  ...
  it('clamps negative values to 0', () => {
    ...
    expect(valueInput.value).toBe('0.0');
  });
});

describe('Value stepper - Percent unit constraints', () => {
  it('clamps values > 100 to 100 when unit is %', () => {
    valueInput.value = '150';
    valueInput.dispatchEvent(new Event('blur'));
    expect(getValue()).toBe(100);
    expect(valueInput.value).toBe('100.0');
  ...
  it('disables increase button when value is 100', () => {
...
describe('Value stepper - Pixel unit constraints', () => {
  ...
  it('allows values > 100 when unit is px', () => {
    ...
    expect(valueInput.value).toBe('500.0');
  });
